...
function Get-DiskInfo {
    ...
    try {
        ...
        Write-Log "Disk found: $($disk.FriendlyName) - $([math]::Round($disk.Size/1GB,2))GB" "SUCCESS"
        return $disk
    }
    catch {
    ...
    try {
        ...
        if ($partitions) {
            ...
            foreach ($partition in $partitions) {
                Write-Log "  Partition $($partition.PartitionNumber): $([math]::Round($partition.Size/1GB,2))GB - $($partition.Type)" "INFO"
            }
        }
        else {
